...
FONT_PATH=sys.argv[-2]; TAG=sys.argv[-1]
...
# Hero sign: plate lab_4:pCube32 is 0.252w x 0.070h, x-centre 0.052, front y~0.244, z[0.298..0.368].
# Place title + subtitle at absolute positions INSIDE the plate (padded) so nothing spills out.
PLATE_CX=0.052; TEXT_Y=0.241
HERO={
 "lab_4:typeMesh1":dict(cz=0.346, w=0.205, h=0.022),  # ROBOTICS LAB (upper)
 "lab_4:typeMesh2":dict(cz=0.316, w=0.180, h=0.011),  # 10TH TO 12TH GRADE (lower)
}

# tame over-bright emission (was: TEXT_GLOW=10, lambert7=5, lambert7.001=50)
...
for n,txt in LABELS.items():
    ...
    if n in HERO:                                   # absolute plate-fit placement
        hl=HERO[n]; s=min(hl['w']/nW, hl['h']/nH)
        cur.extrude=max(0.0008/s,5e-4); nt.scale=(s,s,s)
        nt.location=Vector((PLATE_CX, TEXT_Y, hl['cz']))
    else:                                           # match original footprint
        oW=(mx.x-mn.x)*msc.x; oH=(mx.y-mn.y)*msc.y; oT=(mx.z-mn.z)*msc.z
        s=min(oW/nW,oH/nH); cur.extrude=max(oT/(2*s),5e-4); nt.scale=(s,s,s)
        nt.location=M@cen
...
sc.frame_set(1); sc.render.filepath=os.path.join(OUT,"integrated_%s"%TAG)
bpy.ops.render.render(write_still=True); print("INTEGRATED_DONE",TAG,flush=True)
