...
import { CalendarDays, FileText, Pencil, Trash2, ExternalLink } from "lucide-react";
...
export default function Calendar() {
  ...
  if (isLoading) return <div className="p-8 text-slate-500 animate-pulse text-lg">Loading academic calendars...</div>;
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-slate-800">{editingId ? "Edit Calendar" : "Add New Calendar"}</h3>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            <label className="block text-sm font-medium text-slate-700 mb-1">Title (Classes)</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="e.g., NURSERY TO 12 TH"
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <button type="submit" disabled={saving}
              ...
              {saving ? "Saving..." : editingId ? "Update" : "Add Calendar"}
            </button>
            {editingId && (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-slate-800">Academic Calendars</h3>
        ...
        {items.length === 0 ? (
          <div className="px-6 py-12 text-center text-slate-400">No calendars found.</div>
        ) : (
