  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h3 className="text-lg font-semibold text-slate-800 mb-4">{editingId ? "Edit Book List" : "Add New Book List"}</h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.length > 0 ? items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 text-sm font-bold text-indigo-700">{item.classes}</td>
                ...
                <tr><td colSpan={3} className="px-6 py-12 text-center text-slate-400">No book lists found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
