...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [rows]: any = await pool.query('SELECT image_url FROM facilities WHERE id = ?', [id]);
    if (rows.length === 0) {
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (isUsableUploadedFile(file)) {
      ...
      imageUrl = await saveUploadedFile(file, "facilities");
    }

    await pool.query(
      'UPDATE facilities SET name = ?, image_url = ? WHERE id = ?',
      [name, imageUrl, id]
    );
