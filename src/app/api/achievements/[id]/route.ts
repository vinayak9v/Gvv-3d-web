...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [rows]: any = await pool.query('SELECT image_url FROM achievements WHERE id = ?', [id]);
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [rows]: any = await pool.query('SELECT image_name, image_url FROM achievements WHERE id = ?', [id]);
    if (rows.length === 0) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });

    let imageUrl = rows[0].image_url;
    let imageName = rows[0].image_name;

    if (isUsableUploadedFile(file)) {
      await deleteUploadedFile(imageUrl);
      imageUrl = await saveUploadedFile(file, "achievements");
      imageName = file.name.replace(/\s+/g, '_');
    }

    await pool.query(
      'UPDATE achievements SET category = ?, image_name = ?, image_url = ?, title = ?, details = ? WHERE id = ?',
      [category, imageName, imageUrl, title, details, id]
    );
