...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [rows]: any = await pool.query('SELECT pdf_url FROM curriculums WHERE id = ?', [id]);
    ...
    await pool.query('DELETE FROM curriculums WHERE id = ?', [id]);
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (isUsableUploadedFile(file)) {
      ...
      pdfUrl = await saveUploadedFile(file, "pdf");
    }

    await pool.query(
      'UPDATE curriculums SET title = ?, pdf_url = ? WHERE id = ?',
      [title, pdfUrl, id]
    );
